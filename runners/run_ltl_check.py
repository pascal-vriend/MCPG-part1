...
def print_state_subformulas(states_iterable, gnba_reference, dag, label_prefix=""):
    ...
    for true_idx, state in enumerate(states_iterable):
        layer_info = ""
        ...
        if isinstance(state, tuple) and len(state) == 2:
            ...
            if isinstance(inner_state, (set, frozenset)):
                try:
                    # Lookup the true index in the GNBA parent list
                    g_idx = gnba_states_list.index(inner_state)
                    state_label = f"NBA State N{true_idx} (S{g_idx})"
                except ValueError:
                    state_label = f"NBA State N{true_idx}"
        ...
        # Case 2: Direct GNBA state representation
            ...
            state_label = f"GNBA State S{true_idx}"
        ...
        if raw_set is not None:
            if not raw_set:
                ...
                # Sort the subformulas internally so the set looks neat
...
def run_pipeline(formula_str, build_gnba_flag=True, build_nba_flag=True, run_check_flag=True):
    ...
    def make_acc_pred(u_nid, r_nid):
        return lambda s: u_nid not in s or r_nid in s

    if not until_formulas:
        gnba_acc_sets = {1: lambda s: True}
    else:
        gnba_acc_sets = {
            layer_idx: make_acc_pred(u_nid, dag._nodes[u_nid][2])
        ...
        if is_sat:
            ...
            for idx, (f_set, layer) in enumerate(witness_path):
                ...
                if idx == loop_entry_idx:
                    ...
                    annotation = f" [LOOP BACK TO STEP {loop_entry_idx + 1}]"
    ...
    # -------------------------------------------------------------
    # CASE B: EXPLICIT AUTOMATA MODE (Builds, and saves images)
    # -------------------------------------------------------------
    gnba = None
...
def main():
    ...
    while True:
        ...
        if user_input.startswith("--") or "show-states" in user_input.lower():
            cleaned_cmd = user_input.lower().replace(" ", "")
            if cleaned_cmd in ("--show-states", "show-states=true"):
                SHOW_STATES = True
